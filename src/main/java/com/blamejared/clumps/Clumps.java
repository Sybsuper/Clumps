...
public class Clumps {
    ...
    private void update(TickEvent.WorldTickEvent e) {
        ...
        if (e.world instanceof ServerWorld && !orbs.isEmpty()) {
            List<ExperienceOrbEntity> toremove = new ArrayList<>();
            List<ExperienceOrbEntity> orbscopy;
            synchronized (orbs) {
                orbscopy = new ArrayList<>(orbs);
            }
            for (ExperienceOrbEntity entity : orbscopy) {
            ...
            synchronized (orbs) {
                for (ExperienceOrbEntity experienceOrbEntity : toremove) {
                    orbs.remove(experienceOrbEntity);
                }
            }
        }
    }
